...
jQuery.fn.dataTable.Api.register('MakeCellsEditable()', function(settings) {
	...
	if (table != null) {
		...
		$(table.body()).on('click', 'td', function() {
			...
			if (((settings.columns && settings.columns.indexOf(currentColumnIndex) > -1) || (!settings.columns)) && (settings.inputTypes[currentColumnIndex].type != 'none')) {
...
function getInputHtml(currentColumnIndex, settings, oldValue) {
	...
	if (settings.inputCss || settings.inputTypes[currentColumnIndex].innerCss) {
		inputCss = (settings.inputTypes[currentColumnIndex].innerCss ? settings.inputTypes[currentColumnIndex].innerCss : settings.inputCss);
	}
	...
	case "textarea-confirm":
		...
		break;
	//custome added samarpan
	case "none":
		debugger;
		input.html = "<td>" + oldValue + "</td>";
		break;
	default: // text input
